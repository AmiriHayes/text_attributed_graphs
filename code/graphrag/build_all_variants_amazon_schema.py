#!/usr/bin/env python3
"""
Generic Amazon-schema Steps 3-5 -- parameterized by --dataset so
electronics/toys (identical schema+M1-variant shape to amazon: 38
variants, N7=8/N8=18/N9=12) reuse the exact validated Amazon
build+eval pipeline without duplication.
Usage: python build_all_variants_amazon_schema.py --dataset electronics
"""
import argparse
import sys
import time
...
if __name__ == '__main__':
    ...
    GRAPHS_DIR = REPO_ROOT / f'data/graphrag/graphs/{ds}'
    GRAPHS_DIR.mkdir(parents=True, exist_ok=True)
    OUT_CSV = REPO_ROOT / f'data/graphrag/ragas_results_{ds}.csv'

    client = _client()
    variants = pd.read_csv(REPO_ROOT / f'data/graphrag/all_{ds}_m1_variants.csv')
    questions_df = pd.read_csv(REPO_ROOT / f'data/graphrag/{ds}_20q_subset.csv')
    print(f'{len(variants)} variants, {len(questions_df)} questions')

    dm = GenericDataManager(ds)
    ...
    pd.DataFrame(step1_rows).to_csv(REPO_ROOT / f'data/graphrag/step1_graph_stats_{ds}.csv', index=False)
    print(f'\n\nALL {ds.upper()} VARIANTS COMPLETE')
