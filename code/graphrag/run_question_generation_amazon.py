#!/usr/bin/env python3
"""
Amazon question generation — full 102 (35 single / 33 aggregate / 34 edge_multihop).
Both gates passed (single-hop: mean faithfulness 0.983; edge-multihop: 5/5).
Aggregate uses the aggregate_id family-grouping fallback (co-reviewer overlap
confirmed too sparse: 0 product-pairs share >=2 reviewers in this pool), per
explicit instruction, with text_fidelity_b (product title) only.
"""
import sys
...
if __name__ == '__main__':
    client = _client()
    print('Loading amazon pool (sample_00-09, deduped)...')
    df = load_pooled_samples(dataset='amazon')
    ...
    for i, r in enumerate(single_results):
        all_results.append({'id': f'amazon_sh_{i:03d}', 'dataset': 'amazon', **r, 'multi_hop_stub': None})
    for i, r in enumerate(agg_results):
        all_results.append({'id': f'amazon_agg_{i:03d}', 'dataset': 'amazon', **r, 'multi_hop_stub': None})
    for i, r in enumerate(edge_results):
        all_results.append({'id': f'amazon_edge_{i:03d}', 'dataset': 'amazon', **r, 'multi_hop_stub': None})
