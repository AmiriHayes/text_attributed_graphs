#!/usr/bin/env python3
"""
List 1 -- graph-only proxy scores for ArXiv M1 variants (zero API cost).
Computed directly from existing .pt graph files, no questions/embeddings
needed except link-prediction AUC (reuses data.x, already on disk).
"""
import sys
import time
import warnings
...
def link_prediction_auc(data, seed=SEED) -> float:
    """Mask 10% of edges, score inner product of data.x embeddings for
    masked positives vs an equal number of random non-edges, AUC."""
    rng = np.random.RandomState(seed)
    ei = data.edge_index.numpy()
    # dedupe to undirected unique pairs (i<j)
    pairs = set()
...
def compute_list1(variant: str) -> dict:
    ...
    row = {'variant': variant, 'num_nodes': n, 'num_edges': g.number_of_edges()}
    ...
    # Zero-edge guard (Amazon N8/E10b+E10c are genuinely edgeless -- ground
    # -truth co-participation edges don't exist between reviewers there,
    # confirmed structural fact, not a bug). modularity() divides by
    # deg_sum**2, which is a ZeroDivisionError on an edgeless graph.
    t0 = time.time()
    ...
    # ── Centrality / hub structure ──────────────────────────────────────
    t0 = time.time()
    pr = nx.pagerank(g, seed=SEED) if False else nx.pagerank(g)  # pagerank has no seed param in most versions
    ...
    row['clustering_coeff_mean'] = float(np.mean(list(clustering.values())))
    ...
    # ── Link prediction AUC (uses data.x) ───────────────────────────────
...
def compute_homophily(variant: str, node_type: str, node_lists: dict, label_maps: dict) -> float:
    """Fraction of edges connecting same-categorical_label nodes. Only
    meaningful for N7 (papers carry categorical_label directly); N8 (author)
    nodes have no categorical_label of their own -> NaN."""
    if node_type != 'N7':
        return float('nan')
    pt_path = GRAPHS_DIR / f'{variant}.pt'
    data = torch.load(pt_path, weights_only=False)
...
if __name__ == '__main__':
    print('Loading arxiv pool for node-order / label reconstruction...')
    ...
    for v in variants:
        ...
        print(f'  n={row["num_nodes"]} e={row["num_edges"]} '
              f'largest_cc={row["largest_cc_fraction"]:.3f} modularity={row["modularity"]:.3f} '
              f'pagerank_gini={row["pagerank_gini"]:.3f} link_auc={row["link_pred_auc"]:.3f} '
              f'homophily={row["homophily"]}  ({time.time()-t0:.1f}s)  {row["_timing"]}')
        del row['_timing']
        rows.append(row)

    df = pd.DataFrame(rows)
    out = REPO_ROOT / 'data/graphrag/proxy_list1_arxiv.csv'
